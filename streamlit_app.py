...
timesteps = X_test_scaled.shape[1]
features = X_test_scaled.shape[2]
num_outputs = 2  # Close and Open
samples_to_plot = 10  # number of samples for SHAP
...
X_sample_flat = X_test_scaled[:samples_to_plot].reshape(samples_to_plot, -1)
shap_values = explainer.shap_values(X_sample_flat)  # list of arrays for each output
...
for i in range(num_outputs):
    ...
    assert len(feature_names) == num_features_total, "Feature name count mismatch"
...
shap_df = pd.concat(shap_dfs, axis=0).reset_index(drop=True)
...
# -----------------------------
# Plot SHAP summary per output
# -----------------------------
for i in range(num_outputs):
    sv = shap_values[i].reshape(samples_to_plot, timesteps*features)
    X_flat = X_test_scaled[:samples_to_plot].reshape(samples_to_plot, timesteps*features)
    feature_names = [f"F{f}_t{t}" for t in range(timesteps) for f in range(features)]
    
    fig, ax = plt.subplots()
    shap.summary_plot(sv, X_flat, feature_names=feature_names, show=False,
                      title=f'SHAP Summary Output {i+1}')
    st.pyplot(fig)
