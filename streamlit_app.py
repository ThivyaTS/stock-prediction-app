...
if st.button("🔮 Predict Next Step"):
    ...
    # -----------------------------
    instruction = """Explain how each feature influenced today’s stock price prediction. Use the SHAP importance values to describe whether a feature pushed the predicted price higher or lower 
    compared to the average. Do not mention technical terms like SHAP or the model — instead, explain in plain language. Focus on the relative contribution of each feature, using everyday examples 
    ...
    use percentages or qualitative terms instead.\n\n"""
    
    prompt_lines = [instruction]  # start with instruction
    for feature, values in feature_summary.items():
        line = f"- on {pred_date} ,{feature}: value = {values['value']:.2f}, SHAP importance = {values['shap_importance']:.4f }"
